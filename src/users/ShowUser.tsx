      ...
      <div className="max-w-3xl mx-auto my-10 rounded-xl outline-2 outline-gray-100 bg-amber-50 shadow-2xl shadow-gray-400/70 p-6">
        <div className="flex items-center space-y-5 gap-8">
          ...
          <div className="flex flex-col flex-1 gap-4">
            ...
            {user &&
            user.payload.data &&
            user.payload.data.id.toString() === localStorage.getItem("id") ? (
        ...
        <div className="space-y-2 my-5">
          <h2 className="text-purple-800 font-semibold text-shadow-lg/10">
            <span className="text-gray-900">Name: </span>
            {user && user.payload.data
              ? `${capitalize(user.payload.data.first_name)} ${capitalize(user.payload.data.last_name)}`
              : ""}
          </h2>
          <h2 className="text-purple-700 font-semibold text-shadow-lg/10">
            <span className="text-gray-900">Email: </span>
            {user && user.payload.data ? `${user.payload.data.email}` : ""}
          </h2>

          <p className="text-purple-600 text-center font-semibold text-shadow-lg/10">
            Writer - MyBlog
          </p>
          <p className="text-purple-500 text-center font-semibold text-shadow-lg/10">
            Made {user?.payload.data?.article_count} contributions so far
          </p>
        </div>
